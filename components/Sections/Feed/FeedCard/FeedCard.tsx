    ...
    <Card key={article.post_id} withBorder p="md" radius={0} className={classes.card}>
      <Grid grow>
            ...
            <>
              <Container>
                <Group spacing={'xs'}>
                  ...
                  <Text size={'sm'} weight={400}>Rebbloged by <Text span c="blue" inherit>@{article.reblogged_by[0]}</Text></Text>
                </Group>
                <Space h={'md'} />
              </Container>
            </>
        ...
        <Grid.Col span={isMobile ? 12 : 7}>
          <Container>
            {article?.json_metadata.tags ? article?.json_metadata.tags.slice(0, 3).map?.((item: string) => (
              <Badge mr={5} radius={5} color="gray" key={item} >{item}</Badge>
            )) : null}
          </Container>
        </Grid.Col>
      ...
      }
      {
        isComment &&
        <Grid.Col span={12}>
          <CommentEditor setIsComment={setIsComment} permlink={article.permlink} parentAuthor={article.author} parentPermlink={article?.parent_permlink} />
        </Grid.Col>
      }
    </Card>
  );
