...
export function RecommendedCard({ article }: CardProps) {
  ...
  return (
    <Link href={`community/${article.userpost.community}/post/` + article.userpost.author + '/' + article.userpost.permlink} className={classes.link}>
      <Card p="md" withBorder radius={0} component="a" className={classes.card} sx={{
        '&:hover': {
        ...
        <Grid grow>
          <Grid.Col span={12}>
            <Container className={classes.headerContainer}>
              <Indicator color={'#114f5c'} inline label={article.userpost.author_reputation.toFixed()} size={30} position="bottom-end" withBorder>
                <Avatar color="gray" size={45} radius="xl" src={`https://images.hive.blog/u/${article.userpost?.author}/avatar/`} />
              </Indicator>
